...
def skin_chroma_mask(img):
    """Skin-coloured pixels, WITHOUT the structural guard in ``skin_mask``.

    ``skin_mask`` keeps only components touching the image border, because a
    forearm always enters the frame from an edge and that is what stops a tan
    or leather glove being deleted as skin. But bare skin showing through a
    BREACH in the material is an island in mid-frame, and the guard throws it
    away -- so a detector looking for missing material has to work from the
    unguarded colour test and do its own containment checks instead.
    """
